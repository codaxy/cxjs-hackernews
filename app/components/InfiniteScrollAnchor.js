...
import {findScrollableParent, throttle, browserSupportsPassiveEventHandlers} from "cx/util";
...
export class InfiniteScrollAnchor extends Widget {
    render(context, instance, key) {
        return <InfiniteScrollAnchorComponent
            key={key}
...
class InfiniteScrollAnchorComponent extends VDOM.Component {
    ...
    componentDidMount() {
        ...
        this.onScroll = throttle(() => {
            let depth = scrollableParent.scrollHeight - scrollableParent.scrollTop - scrollableParent.offsetHeight;
            this.props.onMeasure(depth);
        }, 100);

        window.addEventListener("scroll", this.onScroll, browserSupportsPassiveEventHandlers() ? {passive: true} : false);
    }
